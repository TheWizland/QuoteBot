...
class Print(commands.Cog):
    ...
    async def printQuote(ctx, output, authors): #output comes from cur.fetchone()
        if(output is None):
    ...
    async def idQuote(self, ctx, id):
        cur = self.con.cursor()
        cur.execute("SELECT quotes.id, quote, author, quoteRecorder, date, fileExtension FROM authors JOIN quotes on authors.id = quotes.id WHERE authors.id = :id", {"id": id})
        output = cur.fetchone()
        cur.close()
        
        await Print.printQuote(ctx, output, Print.genAuthorString(self.con, id))
    ...
    async def quote(self, ctx, quoteAuthor, numQuotes = 1, *, flags: QuoteFlags):
        try:
            ...
            cur = self.con.cursor()
            cur.execute("SELECT quotes.id, quote, author, quoteRecorder, date, fileExtension FROM authors JOIN quotes ON authors.id = quotes.id AND author = :quoteAuthor WHERE \
            ...
            if(output):
                for quote in output:
                    ...
                    await Print.printQuote(ctx, quote, authors)
                    time.sleep(0.3)
            else:
